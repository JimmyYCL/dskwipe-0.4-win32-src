...
static CString HDSES_systemtime_to_hhmmss(SYSTEMTIME *st) {
  CString rv;
  rv.Format( _T( "%02d:%02d:%02d" ) , st->wHour, st->wMinute, st->wSecond );
...
static CString HDSES_seconds_to_hhmmss(DWORD seconds) {
  CString rv;
	...
	if (hours > 99) {
		...
		if (days > 99) {
			...
			rv.Format(  _T( "%03dd %02dh" ), days, hours );
			return rv;
		}
		//_snprintf(rv, bufsiz, "%02dd%02d%02d", days, hours, minutes);
		return rv;
	}
	rv.Format(  _T( "%02d:%02d:%02d" ), hours, minutes, seconds );
...
//printf("\nsector=%20I64d done_sectors=%20I64d bytes=%20I64d megabytes=%20.10f seconds=%20.10f\n", sector, done_sectors, bytes, megabytes, seconds);
  ...
  CString remaining_time = HDSES_seconds_to_hhmmss(remaining_seconds);
	...
	CString elapsed_time = HDSES_seconds_to_hhmmss( elapsed_seconds );
	...
	CString finish_time = HDSES_systemtime_to_hhmmss(&lpEndTime);
	//systemtime_to_hhmmss(&lpEndTime, finish_time);
...
//static void GetSizeString (LONGLONG size, CString &str) {
static CString HDSES_GetSizeString (LONGLONG size) {
  CString str;
	static CString b, kb, mb, gb, tb, pb;
	...
	if (size > kiloI64 * kilo * kilo * kilo * kilo * 99)
	  str.Format(  _T( "%I64d %s" ), size/ kilo / kilo /kilo/kilo/kilo, pb  );
	  ...
	  str.Format(  _T( "%.1f %s" ), (double)(size/kilod/kilo/kilo/kilo/kilo), pb  );
		
	else if (size > kiloI64*kilo*kilo*kilo*99)
	  str.Format(  _T( "%I64d %s" ), size/kilo/kilo/kilo/kilo, tb  );
		
	else if (size > kiloI64*kilo*kilo*kilo)
	  str.Format(  _T( "%.1f %s" ), (double)(size/kilod/kilo/kilo/kilo), tb  );
		
	else if (size > kiloI64*kilo*kilo*99)
	  str.Format(  _T( "%I64d %s" ), size/kilo/kilo/kilo, gb  );
		
	else if (size > kiloI64*kilo*kilo)
	  str.Format(  _T( "%.1f %s" ), (double)(size/kilod/kilo/kilo), gb  );
		
	else if (size > kiloI64*kilo*99)
	  str.Format(  _T( "%I64d %s" ), size/kilo/kilo, mb  );
		
	else if (size > kiloI64*kilo)
	  str.Format(  _T( "%.1f %s" ), (double)(size/kilod/kilo), mb  );
		
	else if (size > kiloI64)
	  str.Format(  _T( "%I64d %s" ), size/kilo, kb  );
	else
	  str.Format(  _T( "%I64d %s" ), size, b  );
...
static void list_device(char *format_str, char *szTmp, int n) {
	...
	wchar_t partTypeStr[1024] = {0}, *partType = partTypeStr;
	CString size;
	...
	size = HDSES_GetSizeString (diskInfo.PartitionLength.QuadPart);
...
CString HDSES_GetTicksString(CString fmt, ULONGLONG ticks, ULONGLONG tick_frequency)
{
  CString wiping_time, rv;
	DWORD seconds = (DWORD) (ticks / tick_frequency);
	wiping_time = HDSES_seconds_to_hhmmss(seconds);
  rv.Format ( fmt, wiping_time );
  return rv;
}
...
void print_device_info(char *device_name) {
	...
	CString size = HDSES_GetSizeString(total_bytes);
...
int wipe_device(char *device_name, int bytes, int *byte, t_stats *stats, HCRYPTPROV hProv) {
  ...
  //
	// http://stackoverflow.com/questions/27121734/access-denied-error-from-writefile-to-physical-disk-win7
	//
      ...
      )
  {
      ...
      _snprintf(err, sizeof(err), "Error %d attempting to lock device, error code\n",err);
      FatalError(err);
  }
      ...
      )
  {
      ...
      _snprintf(err, sizeof(err), "Error %d attempting to dismount volume, error code\n",err);
      FatalError(err);
  }
...
//  {
//      
//      _snprintf(err, sizeof(err), "Error %d attempting to delete drive layout, error code\n",err);
//      FatalError(err);
//  }
//  
...
//  {
//      
//      _snprintf(err, sizeof(err), "Error %d attempting to refresh partition table, error code\n",err);
//      FatalError(err);
//  }
...
//	}
	...
	if (opt.quiet < 2) {
		if (opt.start != 0 || opt.end != last_sector) {
			...
			//wchar_t size[512];
			CString size = HDSES_GetSizeString(bytes);
	...
	for (unsigned int pass = 1; pass <= opt.passes; ++pass) {
		...
		stats->start_time = HDSES_systemtime_to_hhmmss(&stats->lpStartTime );
...
#error "RANDOM_CRYPTOGRAPHIC has not been implemented yet"
	...
	if (opt.quiet < 2) {
		printf("\n");
		cout << HDSES_GetTicksString( "Wiping time:  %s\n", stats->wiping_ticks, stats->tick_frequency) << endl;
		//print_ticks("Wiping time:  %s\n", stats->wiping_ticks, stats->tick_frequency);

		ULONGLONG elapsed_ticks = get_ticks(stats) - stats->start_ticks;
		cout << HDSES_GetTicksString( "Elapsed time: %s\n", elapsed_ticks, stats->tick_frequency) <<endl;
